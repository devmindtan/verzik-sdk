...
function normalizeUploadPayload(
  ...
  if (typeof Blob === "undefined") {
    throw new Error(
      "This runtime does not support Blob/FormData for multipart/form-data upload.",
    );
  }
...
async function uploadDraftDocument(
  ...
  try {
    ...
    throw new Error(
      `Upload failed: backend returned non-JSON response (${response.status}).`,
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      parsedBody?.status
        ? `Upload failed: ${parsedBody.status}`
  ...
  if (parsedBody.status !== "success" || !parsedBody.document) {
    throw new Error(
      "Upload failed: backend response does not match the expected success schema.",
    );
  }
